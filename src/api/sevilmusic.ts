...
const BASE = 'https://sevilmusics.com';
const AUDIO_HOST = 'dl.sevilmusics.com';
const AUDIO_EXTS = ['.mp3', '.m4a', '.aac'];
const AUDIO_RE = /https?:\/\/dl\.sevilmusics\.com\/[^\s"'<>&\\]+\.(?:mp3|m4a|aac)/gi;
...
let _customProxy = '';
export function setSevilMusicProxy(url: string): void { _customProxy = url; }
...
async function scrapeListingPage(listUrl: string, limit: number): Promise<Track[]> {
  ...
  if (directAudio.length >= limit) {
    return directAudio.slice(0, limit).map((url, i) => ({
      id: `sevilmusic_direct_${i}`,
      title: 'آهنگ سویل موزیک',
      artist: 'SevilMusic',
      album: '',
      duration: 0,
      imageUrl: '',
      audioUrl: url,
      source: 'sevilmusic' as const,
    }));
  }
  ...
  doc.querySelectorAll('a[href]').forEach(el => {
    ...
    if (href && !seen.has(href)) {
      try {
        const u = new URL(href);
        if ((u.hostname === 'sevilmusics.com' || u.hostname === 'www.sevilmusics.com') && u.pathname.includes('/article/')) {
          seen.add(href);
          pageUrls.push(href);
        }
      } catch { /* ignore */ }
  ...
  const results = await Promise.allSettled(
    limited.map(async (pageUrl): Promise<Track | null> => {
      ...
      return {
        ...
        artist: extractArtist(pageDoc) || 'SevilMusic',
        album: '',
        duration: 0,
        imageUrl: cover,
        audioUrl: streams[0],
