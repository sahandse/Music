...
class AudioPlayer {
  ...
  private handleTrackEnd(): void {
    ...
    if (isShuffle && queue.length > 1) {
      ...
      this.playAtIndex(next); return;
    }
    const nextIdx = queueIndex + 1;
    if (nextIdx < queue.length) this.playAtIndex(nextIdx);
    else if (repeatMode === 'all' && queue.length > 0) this.playAtIndex(0);
    else store.updatePlayer({ isPlaying: false });
  }
